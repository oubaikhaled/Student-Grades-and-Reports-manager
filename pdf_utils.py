...
class PDFGenerator:
    ...
    def _get_base_styles():
        ...
        arabic_style = ParagraphStyle(name='ArabicStyle', fontName='ArabicFont', fontSize=12, leading=18, alignment=2)
    ...
    def generate_master_report(cls, homework_title, total_questions, grade_records):
        ...
        elements = [
            ...
            Paragraph(f"Total Questions: {total_questions} | Enrolled Students: {len(grade_records)}",
                      styles["Normal"]),
            Spacer(1, 16)
        ]
    ...
    @classmethod
    def generate_student_report(cls, student_name, homework_title, score, total_questions, percentage, report_text,
                                image_bytes=None):
        buffer = io.BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=letter, rightMargin=36, leftMargin=36, topMargin=36, bottomMargin=36)
        ...
        elements.extend([
            ...
            Spacer(1, 8),
            Paragraph(cls.fix_arabic(
                str(report_text) if pd.notna(report_text) and str(report_text).strip() else "No feedback provided."),
                      arabic_style)
        ])

        if image_bytes:
            elements.extend(
                [Spacer(1, 24), Paragraph("<b>Attached Solution / Notes:</b>", styles["Normal"]), Spacer(1, 8)])
            try:
